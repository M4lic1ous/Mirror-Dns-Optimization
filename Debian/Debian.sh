...
echo "deb https://debian-main.devneeds.ir/ ${codename} main contrib non-free non-free-firmware" >> /etc/apt/sources.list
echo "deb https://debian-main.devneeds.ir/ ${codename}-updates main contrib non-free non-free-firmware" >> /etc/apt/sources.list
echo "deb https://debian-security.devneeds.ir/ ${codename}-security main contrib non-free non-free-firmware" >> /etc/apt/sources.list

echo -e "${BOLD}${NEON_GREEN}✓ Initial mirrors configured:${NC}"
echo -e "  ${NEON_CYAN}Main: https://debian-main.devneeds.ir/${NC}"
echo -e "  ${NEON_CYAN}Security: https://debian-security.devneeds.ir/${NC}"
...
if [[ ! "$line" =~ /debian/$ ]] && [[ ! "$line" =~ /debian$ ]]; then
line="${line%/}/"
fi
mirrors+=("$line")
fi
