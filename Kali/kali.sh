...
local codename=$(get_kali_codename)
echo -e "${BOLD}${NEON_AQUA}▶ Setting up initial mirrors for dependency installation...${NC}"

> /etc/apt/sources.list

echo "deb https://kali.devneeds.ir/ ${codename} main non-free contrib" >> /etc/apt/sources.list

echo -e "${BOLD}${NEON_GREEN}✓ Initial mirrors configured:${NC}"
echo -e "  ${NEON_CYAN}Kali Mirror: https://kali.devneeds.ir/${NC}"
echo -e "  ${NEON_CYAN}Branch: ${codename}${NC}"
...
local codename=$(get_kali_codename)
echo "deb ${BEST_MIRROR} ${codename} main non-free contrib" > "/etc/apt/sources.list"
